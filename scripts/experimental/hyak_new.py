...
l = 10
ffn_factor = 8
ffn = ffn_factor*model

seq_len = 512
batch_mult = 1
batch_size = batch_mult*8*1024

update_freq = 24//gpus
remainder = 24/(update_freq*gpus)
max_updates = int(1.0*16000*remainder/batch_mult)

total_tokens = batch_size*update_freq*max_updates*gpus

params = 0
params += ffn*model*2 # ffn
params += model*model # attn output
params += 3*model*model # attn proj

param_per_layer = params
params *= l
...
time_hours = min(int(math.ceil(seconds/60/60*1.2))+2, 4)


print('='*80)
print(f'Total GPU days: {seconds/60/60/24*gpus:.2f} days')
print(f'Total FLOPS: {FLOPS:.2E}')
print(f'Total_time: {seconds/60/60:.2f}h')
print(f'Parameters per layer: {param_per_layer/1e9:.3f}b')
print(f'Parameters: {params/1e9:.3f}b')
print(f'Tokens per parameter: {total_tokens/params:.3f}')
print(f'Tokens: {total_tokens/1e9:.3f}b')
print(f'Expected tokens per second: {tokens_per_sec:.0f}')
print(f'Expected runtime: {seconds/60/60:.2f}h')
print('='*80)
...
args3[key] = []
args3[key] = []
#for ffn_dim, model_dim in zip([4*model, 8*model, 2*model, 12*model], [model, model//4*3, model//4*5, model//8*5]):
for ffn_dim, model_dim in zip([ffn_factor*model], [model]):
    for heads in [model_dim//128]:
        args3[key].append((model_dim, ffn_dim, heads, model_dim, model_dim))
...
args2['bashcmd'] = '"cp -r /gscratch/cse/data/cc_small /tmp/"'
...
args3[('max-update', 'warmup-updates', '')] = [(max_updates, 3000, ' /tmp/cc_small')]
...
if not args.dry:
    s.run_jobs(begin=begin, gpus_per_node=gpus_per_node, requeue=True, as_array=False)
