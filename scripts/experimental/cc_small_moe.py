...
for layers in [10, 20]:
    for model_dim in [1024, 2048]:
        heads = 8*(model_dim//512)
        for ff_dim in [8192, 8192*2]:
            if layers == 20 and ff_dim == 8192*2: continue
            if layers == 10 and ff_dim == 8192: continue
            if layers == 10 and ff_dim == 1024: continue
...
args3[('max-tokens', 'update-freq', 'tokens-per-sample')].append((2048, (1024//512)*128//gpus, 512))
args3[('max-tokens', 'update-freq', 'tokens-per-sample')].append((2048, (1024//1024)*128//gpus, 1024))
...
args3[key].append((0.00163*0.5, 0.0))
args4 = []
