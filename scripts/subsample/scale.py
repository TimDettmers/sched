...
logfolder = 'subsample/{0}'.format(name)
ckp_name = logfolder
cores_per_job = 5
...
args3[('max-tokens', 'update-freq', 'tokens-per-sample')].append((2048*8, 16//gpus, 512))
args3[('max-update', 'warmup-updates', '')] = [(16000, 3000, ' /tmp/cc_small')]
...
for seed in range(num_seeds):
    ...
    for arg4 in args4:
        ...
        for i, values in enumerate(args_prod):
            ...
            cmds = ['cp -r /gscratch/cse/data/cc_small /tmp/cc_small', job_cmd]
...
if not args.dry:
    s.run_jobs(begin=begin, gpus_per_node=gpus_per_node)
