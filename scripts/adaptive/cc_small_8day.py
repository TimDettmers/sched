...
logfolder = 'lm/cc_small/{0}'.format(name)
ckp_name = logfolder
cores_per_job = 2
...
args3[key] = []
for model_dim in [1024+512, 2048]:
    heads = 8*(model_dim//512)
    for ff_dim in [16384, 16384+4096]:
...
args3[('max-tokens', 'update-freq', 'tokens-per-sample')].append((4096, 64//gpus, 512))
#args3[('max-tokens', 'update-freq', 'tokens-per-sample')].append((4096, 128//gpus, 512))
args3[('dropout', 'attention-dropout', 'relu-dropout')] = [(0.0, 0.0, 0.0)]
args3['clip-norm'] = [0.6]

args3[('max-update', 'warmup-updates', '')] = [(24000, 3000, ' data/cc_news_small')]

args3['weight-decay'] = [0.00]

key = ('lr', 'max-lr', 'min-lr', 'warmup-init-lr')
args3[key] = []
for params in [1e7,1e6, 1e5]:
