...
args2['use-bnb'] = ''
args2['optim-bits'] = 8
args3['bnb-analysis'] = ['linear', 'dynamic', 'dynamic-blockwise']

args3[('no-scale-embedding', 'stable-emb')] = [(True, True)]
    ...
    for arg4 in args4:
        ...
        for i, values in enumerate(args_prod):
            ...
            if any([k in job_cmd for k in args5.keys()]):
                    ...
                    if substr in job_cmd:
                        for key, values in pdict.items():
                            for v in values:
                                ...
                                job_cmd5 = job_cmd5 + ' --analysis-dir {0}/analysis/'.format(checkpoint_dir)
                                cmds = [job_cmd5]
                ...
                job_cmd = job_cmd + ' --analysis-dir {0}/analysis/'.format(checkpoint_dir.strip())
                cmds = [job_cmd]
                if rdm.rand(1) <= args.p:
                    jobs.append(job_cmd)
                    s.add_job(logfolder, repo, change_dir, cmds, time_hours, fp16, cores=cores_per_job, mem=mem, constraint=constraint, exclude=exclude, time_minutes=time_minutes, gpus=gpus)
