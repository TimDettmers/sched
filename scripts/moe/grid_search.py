...
fp16 = True
args3 = {}
args3['num-experts'] = [8, 32]
args3['moe-ff-dim'] = [args2['decoder-input-dim'], args2['decoder-input-dim']*2]
args3['experts-per-seq'] = [3, 31]
args3['moe-freq'] = [2]
args3['iloss-weight'] = [0.1, 0.01]
args3['bloss-type'] = ['mean-prob']
args3['lr'] = [0.003, 0.006]
args3['weight-decay'] = [1e-07]
args3['dropout'] = [0.2, 0.3]
args3['attention-dropout'] = [0.1]
args3['activation-dropout'] = [0.1]
args3['lamb-betas'] = ["'(0.9, 0.999)'"]
args3['clip-norm'] = [0.1, 1.0]
args3['sample'] = [0, 1, 2]
args3['epsilon'] = [0.2, 0.0]
args3['epsilon-min'] = [0.0]
args3['epsilon-length'] = [1/4]
args3['counter-reset-period'] = [500, 2500]
#args3[('decoder-embed-dim','decoder-attention-heads')] = [(128, 4), (256, 8)]
#args3[('max-update', '', 'warmup-updates')] = [(15000//gpus, 'data/wikitext-2', 200//gpus), (25000//gpus, 'data/wikitext-5', 500//gpus), (50000//gpus, 'data/wikitext-10', 1000//gpus)]
#args3[('max-update', '', 'warmup-updates')] = [(15000//gpus, 'data/wikitext-2', 200//gpus)]#, (25000//gpus, 'data/wikitext-5', 500//gpus), (50000//gpus, 'data/wikitext-10', 1000//gpus)]
args3[('max-update', '', 'warmup-updates', 'update-freq')] = []
args3[('max-update', '', 'warmup-updates', 'update-freq')].append((50000//gpus, 'data/wikitext-10', 5000//gpus, 1))
#args3[('max-update', '', 'warmup-updates', 'update-freq')].append((10000//gpus, 'data/wikitext-2', 1000//gpus, 1))

#args3[('max-update', '', 'warmup-updates')] = [(10000//gpus, 'data/wikitext-2', 1000//gpus), (10000//gpus, 'data/wikitext-2', 2000//gpus)]
