...
s = gpuscheduler.Scheduler('/home/tim/data/git/sched/config/', verbose=args.verbose)
...
cmd_cifar = 'OMP_NUM_THREADS=1 python main.py --model {4} --decay_frequency 30000 --batch-size 128 --data cifar --epochs 250 --density {5} --seed {3} --prune {0} --growth {1} --redistribution {2} --verbose --prune-rate 0.2 {6}'
...
models_and_densities = [('vgg-d', 0.05), ('vgg-c', 0.05), ('vgg-like', 0.03)]
...
for seed in range(num_seeds):
    for (model, density) in models_and_densities:
        ...
        growth = 'momentum'
        redist = 'momentum'
        jobs.append(['momentum_reset_vgg_prune_0.2/{0}/{1}'.format(model, density), 'sparse_learning/mnist_cifar/', cmd.format(prune, growth, redist, seed+seed_offset, model, density, ' --fp16' if fp16 else ''), fp16])
