...
cmd = ' python run_distributed_engines.py     hydra.verbose=true     config=pretrain/moco/moco_1node_resnet     config.DATA.TRAIN.DATA_PATHS="[/datasets01/imagenet_full_size/061417/]"     '
args2 = {}

name = 'mocov1_vissl_grid1'
constraint = 'volta'
...
change_dir = '/private/home/timdettmers/git/vissl/'
repo = 'vissl'
exclude = ''
...
#args3[('adam-bits', 'percentile-clipping', 'adam8bits-method')] = [(8, 100, 'quantile'), (8, 100, 'dynamic_tree')]

#args3[('optimizer', 'adam-bits', 'percentile-clipping', 'adam8bits-method')] = [('momentum', 8, 100, 'quantile'), ('momentum', 8, 100, 'dynamic_tree'), ('sgd', 32, 100, 'dynamic_tree')]


args4 = []
args4.append('  config.OPTIMIZER.name=sgd config.OPTIMIZER.adam8bits_method=dynamic_tree ')
args4.append('  config.OPTIMIZER.name=momentum config.OPTIMIZER.adam8bits_method=dynamic_tree ')
args4.append('  config.OPTIMIZER.name=momentum config.OPTIMIZER.adam8bits_method=quantile ')
...
for seed in range(num_seeds):
    ...
    for arg4 in args4:
        ...
        for i, values in enumerate(args_prod):
            ...
            #save_dir = ' --output-dir {0}'.format(checkpoint_dir)
            #save_dir2 = ' --save-dir {0}'.format(checkpoint_dir2)
            save_dir = 'config.CHECKPOINT.DIR="{0}"'.format(checkpoint_dir.strip())
